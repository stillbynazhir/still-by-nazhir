...
const GALLERY_WORKER_URL = process.env.REACT_APP_GALLERY_WORKER_URL || "";
  ...
  return (
    ...
    >
      <button onClick={onClose} style={{
        ...
        border: "none", color: "#666", fontSize: 28, cursor: "pointer",
        fontFamily: "'Outfit', sans-serif", fontWeight: 300, lineHeight: 1,
      }}>×</button>
      <button onClick={(e) => { e.stopPropagation(); onPrev(); }} style={{
        position: "absolute", left: 16, top: "50%", transform: "translateY(-50%)",
        background: "none", border: "none", color: "#555",
        fontSize: 32, cursor: "pointer", padding: 12,
      }}>‹</button>
...
/* ── Gallery Page ── */
function GalleryPage({ isMobile, onBack }) {
  ...
  useEffect(() => {
    if (!GALLERY_WORKER_URL) {
      setFetchError("Gallery worker URL not configured.");
      setLoading(false);
      return;
    }
  ...
  return (
    <div style={{ minHeight: "100vh", paddingTop: isMobile ? 80 : 100 }}>
      ...
      <div style={{ padding: isMobile ? "32px 24px 64px" : "48px 48px 100px" }}>
        <button
          onClick={onBack}
          style={{
            ...
            marginBottom: 40, display: "inline-flex", alignItems: "center", gap: 8,
        ...
        {loading && (
          <div style={{ textAlign: "center", padding: "80px 0" }}>
            <p style={{ fontFamily: "'Outfit', sans-serif", fontSize: 12, color: "#333", letterSpacing: "0.1em" }}>Loading...</p>
          </div>
        )}

        {fetchError && (
          <div style={{ textAlign: "center", padding: "80px 0" }}>
            <p style={{ fontFamily: "'Outfit', sans-serif", fontSize: 13, color: "#555" }}>{fetchError}</p>
          </div>
        )}

        {!loading && !fetchError && filteredGallery.length === 0 && (
          <div style={{ textAlign: "center", padding: "80px 0" }}>
            <p style={{ fontFamily: "'Outfit', sans-serif", fontSize: 13, color: "#333" }}>No photos in this category yet.</p>
          </div>
        )}
...
export default function StillByNazhir() {
  ...
  const goToGallery = () => { setPage("gallery"); window.scrollTo({ top: 0, behavior: "smooth" }); };
  ...
  const handleSubmit = (e) => {
    ...
    .then(() => {
      emailjs.send(EMAILJS_SERVICE_ID, "template_zridmhv", {
        from_name: formData.name,
        reply_to: formData.email,
      }, EMAILJS_PUBLIC_KEY);
      setSubmitted(true);
  ...
  return (
    <div style={{ background: "#080808", color: "#fff", fontFamily: "'Georgia', serif", minHeight: "100vh" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .gallery-tab {
          ...
          padding: 10px 20px; transition: color 0.3s, border-color 0.3s;
          border-bottom: 1px solid transparent;
        }
        .gallery-tab:hover { color: #888; }
        .gallery-tab.active { color: #fff; border-bottom-color: #fff; }
        .gallery-img-wrap {
          overflow: hidden; position: relative; cursor: zoom-in; background: #0d0d0d;
        }
        ...
        .gallery-img-wrap:hover .gallery-overlay { background: rgba(8,8,8,0.2); }
      ...
      <nav style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          {!isMobile && (
            <div style={{ display: "flex", gap: 36, alignItems: "center" }}>
              {page === "gallery" ? (
                <>
                  <span className="nav-link" onClick={goHome}>Home</span>
                  <span className="nav-link" style={{ color: "#fff" }}>Gallery</span>
                </>
              ) : (
                NAV_LINKS.map(l => (
          ...
          {isMobile ? (
            ...
            <a href={page === "gallery" ? undefined : "#contact"}
              onClick={page === "gallery" ? goHome : undefined}
              style={{
        ...
        {isMobile && mobileMenuOpen && (
          <div style={{ animation: "slideDown 0.25s ease forwards", paddingTop: 8, paddingBottom: 8 }}>
            {page === "gallery" ? (
              <>
                <span className="mobile-nav-link" onClick={() => { setMobileMenuOpen(false); goHome(); }}>Home</span>
                <span className="mobile-nav-link" style={{ color: "#fff" }}>Gallery</span>
              </>
            ) : (
              NAV_LINKS.map(l => (
                <a key={l} href={`#${l.toLowerCase()}`} className="mobile-nav-link"
                  onClick={() => setMobileMenuOpen(false)}>{l}</a>
              ))
            )}
            <a href="#contact" onClick={() => { setMobileMenuOpen(false); if (page === "gallery") goHome(); }}
              style={{
      ...
      {/* PAGE CONTENT */}
      {page === "gallery" ? (
        <GalleryPage isMobile={isMobile} onBack={goHome} />
      ) : (
        <>
          {/* HERO */}
          <section style={{ position: "relative", height: "100vh", paddingTop: 50, display: "flex", alignItems: "flex-end", overflow: "hidden" }}>
            ...
            <div style={{ position: "relative", zIndex: 2, padding: isMobile ? "0 24px 56px" : "0 48px 72px", width: "100%" }}>
              <div style={{ opacity: 1, animation: "fadeDown 1.2s ease forwards" }}>
                <p style={{ fontFamily: "'Outfit', sans-serif", fontSize: 11, fontWeight: 500, letterSpacing: "0.22em", color: "#555", textTransform: "uppercase", marginBottom: 20 }}>Philadelphia, PA</p>
                <h1 style={{
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 24 }}>
                  <a href="#contact" style={{ fontFamily: "'Outfit', sans-serif", fontSize: 11, fontWeight: 600, letterSpacing: "0.16em", textTransform: "uppercase", color: "#fff", textDecoration: "none", borderBottom: "1px solid #444", paddingBottom: 2 }}>Inquire Now</a>
                  <a href="#portfolio" style={{ fontFamily: "'Outfit', sans-serif", fontSize: 11, letterSpacing: "0.16em", textTransform: "uppercase", color: "#444", textDecoration: "none" }}>View Work ↓</a>
                </div>
              </div>
            </div>
            {!isMobile && (
          ...
          {/* STATS BAR */}
          <FadeIn>
            <div style={{ borderTop: "1px solid #111", borderBottom: "1px solid #111", display: "flex", justifyContent: "center", alignItems: "center", gap: isMobile ? 32 : 64, padding: isMobile ? "28px 24px" : "36px 48px", flexWrap: "wrap" }}>
              {STATS.map((s, i) => (
          ...
          <section id="portfolio" style={{ padding: isMobile ? "64px 24px" : "100px 48px" }}>
            <FadeIn>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 40, flexWrap: "wrap", gap: 16 }}>
                ...
                <span onClick={goToGallery} style={{ fontFamily: "'Outfit', sans-serif", fontSize: 10, color: "#555", letterSpacing: "0.14em", textTransform: "uppercase", borderBottom: "1px solid #222", paddingBottom: 2, cursor: "pointer", transition: "color 0.3s, border-color 0.3s" }}
            ...
            <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr 1fr", gap: 3 }}>
              {[
                ...
                <FadeIn key={i} delay={i * 0.1} style={{ gridColumn: item.size }}>
                  <div
                    style={{ height: item.height, background: "#0d0d0d", position: "relative", overflow: "hidden", cursor: "pointer" }}
                    onClick={goToGallery}
                  ...
                  >
                    <img src={process.env.PUBLIC_URL + "/" + item.src} alt={item.label} style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: item.pos, display: "block" }} />
                    <div className="label" style={{ position: "absolute", inset: 0, background: "rgba(8,8,8,0.55)", display: "flex", alignItems: "flex-end", padding: "20px 24px", opacity: isMobile ? 1 : 0, transition: "opacity 0.3s ease" }}>
          ...
          <section id="about" style={{ padding: isMobile ? "64px 24px" : "100px 48px", borderTop: "1px solid #111" }}>
            <div style={{ maxWidth: 900, margin: "0 auto", display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: isMobile ? 40 : 80, alignItems: "center" }}>
              <FadeIn>
                <div style={{ aspectRatio: "3/4", overflow: "hidden", background: "#0d0d0d", maxWidth: isMobile ? 320 : "100%", margin: isMobile ? "0 auto" : 0 }}>
                  <img src={process.env.PUBLIC_URL + "/portrait-1.jpg"} alt="Nazhir Jackson" style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: "center top", display: "block" }} />
                </div>
              </FadeIn>
              <FadeIn delay={isMobile ? 0 : 0.2}>
                ...
                <div style={{ marginTop: 36, display: "flex", gap: 24 }}>
                  ...
                  <a href="https://instagram.com/stillbynazhir" target="_blank" rel="noreferrer" style={{ fontFamily: "'Outfit', sans-serif", fontSize: 11, letterSpacing: "0.14em", textTransform: "uppercase", color: "#444", textDecoration: "none" }}>@stillbynazhir ↗</a>
                </div>
              </FadeIn>
            </div>
          </section>
              ...
              {submitted ? (
                ...
                <FadeIn delay={0.15}>
                  <form onSubmit={handleSubmit}>
                    ...
                    {error && <p style={{ fontFamily: "'Outfit', sans-serif", fontSize: 12, color: "#c0392b", marginBottom: 16 }}>{error}</p>}
                    <button type="submit" className="submit-btn" disabled={sending} style={{ width: isMobile ? "100%" : "auto" }}>
      ...
      {/* FOOTER */}
      <footer style={{ borderTop: "1px solid #111", padding: isMobile ? "28px 24px" : "36px 48px", display: "flex", justifyContent: isMobile ? "center" : "space-between", alignItems: "center", flexDirection: isMobile ? "column" : "row", gap: isMobile ? 12 : 0 }}>
        ...
        <a href="https://instagram.com/stillbynazhir" target="_blank" rel="noreferrer" style={{ fontFamily: "'Outfit', sans-serif", fontSize: 10, color: "#949494", letterSpacing: "0.12em", textTransform: "uppercase", textDecoration: "none" }}>Instagram ↗</a>
      </footer>
    </div>
  );
}